...
public class CustomUsernameAuthenticationProvider implements AuthenticationProvider {
    ...
    public CustomUsernameAuthenticationProvider(UserService userService, PasswordEncoder passwordEncoder) {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        if(!passwordEncoder.matches(password, userDetails.getPassword())){
            throw new BadCredentialsException(String.format("username: %s, password: %s", username,password));
        }
